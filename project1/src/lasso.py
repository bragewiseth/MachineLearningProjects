...
model = Lasso(fit_intercept=False, alpha=Lambda)
clf = model.fit(X_train, y_train)
y_tilde = clf.predict(myScaler.transform(poly.transform(X))) + y_train_mean
y_tilde = y_tilde.reshape(n,n)
...
surf = ax.plot_surface(xx, yy, y_tilde, cmap=cm.coolwarm, # type: ignore
                       linewidth=0, antialiased=False)
